// Host resolution: the client knows one bootstrap host and resolves its per-user/per-region
// endpoints from GET /.well-known/agentage-sync, caching for the response ttl (default 1h).
// fetch + clock are INJECTED so it unit-tests without a live server. A memory is on EITHER
// the git channel or the couch channel, never both.

// A couch-channel memory advertised in the resolution: its per-memory db on the shared
// cluster. The JWT is NOT here - the client mints it from `couchTokenUrl` (the auth service
// is the sole minter).
...
export interface SyncResolution {
  ...
  // `vaults` (git) or `couchVaults` (couch), never both. Absent => git-only shape, parsed
  // exactly as before (old-server back-compat).
...
// Server field names: couch_endpoint, couch_token_url, couch_vaults[{ vault, db }]. Parsed
// only when all three are present + well-formed, so a partial/old payload degrades to git
// rather than half-advertising a couch channel.
...
export class HostResolver {
  ...
  constructor(
    private readonly bootstrapHost: string, // e.g. https://sync.<fqdn>
