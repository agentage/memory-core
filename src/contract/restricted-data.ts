...
// or "auth" pass, while an actual `sk-...`, `password: hunter2`, SSN, or card number is
// refused before it is ever persisted. A screen for obvious secrets, not a DLP system;
// the write/edit paths refuse (never silently redact) on a hit.
...
const hasSecretAssignment = (text: string): boolean => {
  ...
  return m != null && !PLACEHOLDER_RE.test(m[1]);
};
...
// The stable, greppable refusal message. Same wording the cloud MCP returns, so every
// surface (CLI verbs, local MCP, cloud) refuses identically.
...
// Thrown by the write/edit paths when input carries a restricted data class. The MCP
// layer maps it to an isError tool result; CLI verbs surface `message` to the user.
