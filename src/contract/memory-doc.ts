// Pure, storage-agnostic document helpers: how a path becomes a title, how tags
// are derived, how a document serializes/parses, and how a search snippet is cut.
...
// The on-disk document shape, so a read response is byte-identical to the stored
// .md (files-first). Empty frontmatter emits the bare body, no fences.
...
// Title = filename stem, never a frontmatter `title` (Obsidian fidelity).
...
// Indexed tags = union(frontmatter `tags`, inline `#tags`), numeric-only dropped.
