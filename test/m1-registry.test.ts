...
// M1-R2 + TC7 - registry builds LocalBackend per path entry, path-only = zero network.
describe('M1-R2 createRegistry', () => {
...
// TC7 - a path-only vault performs ZERO network calls across its full lifecycle.
describe('TC7 path-only vault makes no network calls', () => {
